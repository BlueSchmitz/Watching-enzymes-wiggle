    ...
    for acc_idx in nearby_acc:
        ...
        for H in bonded_H:
            ...
            if nearby_waters:
                ...
                Ow_pos = water_pos[nearby_waters]  # (W, 3)

                # Vector from H -> Ow and Ow -> A
                HOw_vec = Ow_pos - H_pos           # (W, 3)
                OwA_vec = Ow_pos - A_pos           # (W, 3)

                # distances
                dist_HOw = np.linalg.norm(HOw_vec, axis=1)
                dist_OwA = np.linalg.norm(OwA_vec, axis=1)

                mask = (dist_HOw <= HOW_DIST_CUTOFF) & (dist_OwA <= OWA_DIST_CUTOFF)

                if not np.any(mask):
                    continue

                Ow_pos_f = Ow_pos[mask]
                HOw_vec_f = HOw_vec[mask]
                OwA_vec_f = OwA_vec[mask]
                dist_HOw_f = dist_HOw[mask]
                dist_OwA_f = dist_OwA[mask]

                # CH–Ow angle
                CH_vec = H_pos - C2_pos        # (3,)
                CH_vec = CH_vec / np.linalg.norm(CH_vec)

                CHOw_vec = Ow_pos_f - H_pos    # (W,3)
                CHOw_norm = np.linalg.norm(CHOw_vec, axis=1, keepdims=True)
                CHOw_unit = CHOw_vec / (CHOw_norm + 1e-12)

                cos_CHOw = np.sum(CH_vec * CHOw_unit, axis=1)
                angle_CHOw = np.degrees(np.arccos(np.clip(cos_CHOw, -1, 1)))

                # H–Ow–A angle
                HOw_vec = H_pos - Ow_pos_f
                HOw_vec = HOw_vec / np.linalg.norm(HOw_vec, axis=1, keepdims=True)

                OwA_vec = A_pos - Ow_pos_f
                OwA_vec = OwA_vec / np.linalg.norm(OwA_vec, axis=1, keepdims=True)

                cos_HOwA = np.sum(HOw_vec * OwA_vec, axis=1)
                angle_HOwA = np.degrees(np.arccos(np.clip(cos_HOwA, -1, 1)))
