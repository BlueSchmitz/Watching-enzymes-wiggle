...
def createOutputFile(template_file, frame_number, com_distance, search_string="XXX"):
    """
    Look for instances of the search string in a template file and replace with
    the frame number in a new file. Save in a subdirectory named after the COM distance.
    """

    # Create directory named after the COM distance
    directory_name = "COM_%.3f" % com_distance
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)

    # Define output file path
    out_file = os.path.join(directory_name, "frame-%i_%s" % (frame_number, "run-umbrella.sh"))
    ...
    if len(template_files) != 0:
        ...
        for t in template_files:
            for i in sampled_indexes:
               ...
               com_distance = distance_table[i][1]
               createOutputFile(t, frame, com_distance, search_string="XXX")
