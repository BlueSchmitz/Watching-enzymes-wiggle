...
# calculate distance between Lys168 NZ and Leu238 CD over time, output in xvg format
apptainer exec $GROMACS_CONTAINER gmx_mpi distance -s $tpr -f md_fit.xtc -n index.ndx -oall lys168_leu238_distance.xvg -tu ns -select 'com of group 24 plus com of group 25'
python $scripts/plotxvg.py lys168_leu238_distance.xvg
# histogram of the distance between Lys168 NZ and Leu238 with bin width of 0.2 nm, output in xvg format
apptainer exec $GROMACS_CONTAINER gmx_mpi analyze -f lys168_leu238_distance.xvg -dist lys168_leu238_hist.xvg -bw 0.2
python $scripts/plotxvg_hist.py lys168_leu238_hist.xvg
...
python $scripts/plot_RMSF_red_Bt.py rmsf_Ca.xvg
### define frames with distance between Lys168 NZ and Leu238 < 0.6 nm as "closed" and >= 0.6 nm as "open", output in xvg format
# Compute distance time series (ps)
apptainer exec $GROMACS_CONTAINER gmx_mpi distance -f md_fit.xtc -s $tpr -n index.ndx -select 'com of group 24 plus com of group 25' -oall dist_k168_l238_ps.xvg
...
# Clustering 
python $scripts/run_clustering_Bt.py $tpr md_fit.xtc
