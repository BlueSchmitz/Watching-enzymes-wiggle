...
apptainer exec $GROMACS_CONTAINER mpirun -n 1 gmx_mpi mdrun -deffnm em
...
apptainer exec $GROMACS_CONTAINER mpirun -n 1 gmx_mpi mdrun -deffnm nvt -cpt 15 
...
do
  ...
  apptainer exec $GROMACS_CONTAINER mpirun -n 1 gmx_mpi mdrun -deffnm npt_${i} -cpt 15
