...
sed -i '/#ifdef POSRES/,/#endif/d' topol_Protein_chain_A_5.itp # Remove the protein restraint block (POSRES) from chain A itp
...
python3 $scripts/scale_residues.py > processed_scaled_dimer.top
python3 $scripts/scale_residues.py Protein_chain_A > processed_scaled_monomer.top
echo "Generated processed_scaled_monomer.top and processed_scaled_dimer.top with selected residues for scaling."
